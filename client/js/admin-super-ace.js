...
const fields = {
  ...
  freeSpinsEnabled: document.getElementById("freeSpinsEnabled"),
    aceMeterEnabled:
    document.getElementById("aceMeterEnabled"),
  ...
  freeSpinsMultiplier:
    document.getElementById("freeSpinsMultiplier"),
      aceMeterTrigger:
    document.getElementById("aceMeterTrigger"),

  aceMeterMultiplier:
    document.getElementById("aceMeterMultiplier"),
  dailyPayoutLimit:
...
function fillSettings(settings) {
  ...
  fields.freeSpinsMultiplier.value =
    settings.freeSpinsMultiplier ?? 1;
      ...
      fields.aceMeterTrigger.value =
    settings.aceMeterTrigger ?? 5;

  fields.aceMeterMultiplier.value =
    settings.aceMeterMultiplier ?? 3;
  ...
  return {
    ...
    freeSpinsEnabled:
      fields.freeSpinsEnabled.checked,
          ...
          aceMeterEnabled:
      fields.aceMeterEnabled.checked,
    ...
    freeSpinsMultiplier:
      Number(fields.freeSpinsMultiplier.value),
          ...
          aceMeterTrigger:
      Number(fields.aceMeterTrigger.value),

    aceMeterMultiplier:
      Number(fields.aceMeterMultiplier.value),
...
function validateSettings(settings) {
  ...
  if (
    ...
    if (
    !Number.isInteger(settings.aceMeterTrigger) ||
    settings.aceMeterTrigger < 1 ||
    settings.aceMeterTrigger > 100
  ) {
    throw new Error(
      "Ace Meter Target অবশ্যই 1 থেকে 100-এর মধ্যে হবে।",
    );
  }

  if (
    !Number.isFinite(settings.aceMeterMultiplier) ||
    settings.aceMeterMultiplier < 1 ||
    settings.aceMeterMultiplier > 100
  ) {
    throw new Error(
      "Ace Bonus Multiplier অবশ্যই 1x থেকে 100x-এর মধ্যে হবে।",
    );
  }
}
...
async function saveSettings() {
  ...
  try {
    ...
    showToast(
      "Super Ace settings সফলভাবে save হয়েছে।",
      "Saved",
    );
...
async function loadDashboard() {
  const responseData = await apiRequest(
    "/api/admin/super-ace/dashboard?limit=100",
  );
...
async function refreshAll() {
  ...
  try {
    ...
    showToast(
      error.message || "Super Ace dashboard load হয়নি।",
      "Loading Error",
    );
  } finally {
