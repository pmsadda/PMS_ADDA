...
const aviatorStage =
  document.getElementById("aviatorStage");

const bettingCountdown =
  document.getElementById("bettingCountdown");
...
const multiplierStatus =
  document.getElementById("multiplierStatus");

const crashedMessage =
  document.getElementById("crashedMessage");

const crashedMultiplier =
  document.getElementById("crashedMultiplier");

const crashHistory =
  document.getElementById("crashHistory");

const serverSeedHash =
  document.getElementById("serverSeedHash");

const serverSeed =
  document.getElementById("serverSeed");

const fairCrashPoint =
  document.getElementById("fairCrashPoint");
...
function refreshBetPanel(slot) {
  ...
  if (elements.buttonAmount) {
    elements.buttonAmount.textContent =
      money(amount);
  }
  ...
  if (!bet) {
    ...
    if (elements.amountInput) {
      elements.amountInput.disabled =
        roundState !== "betting";
    }

    if (elements.autoEnabled) {
      elements.autoEnabled.disabled =
        roundState !== "betting";
    }
...
function startBettingCountdown() {
  ...
  if (!Number.isFinite(endTime)) {
    bettingCountdown?.classList.add(
      "is-hidden",
    );
  ...
  bettingCountdown?.classList.remove(
    "is-hidden",
  );
...
function updateFairness(round) {
  if (!round) {
    ...
    if (serverSeed) {
      serverSeed.textContent =
        "Hidden until crash";
    }

    if (fairCrashPoint) {
      fairCrashPoint.textContent =
        "Hidden";
    }
...
function updateRoundUI() {
  ...
  if (!round) {
    ...
    aviatorStage?.classList.remove(
      "flying",
    );

    crashedMessage?.classList.add(
      "is-hidden",
    );
  ...
  if (status === "crashed") {
    ...
    if (crashedMultiplier) {
      crashedMultiplier.textContent =
        multiplierText(
          currentMultiplier,
        );
    }

    crashedMessage?.classList.remove(
      "is-hidden",
    );
  }
...
function addCrashHistory(value) {
  ...
  item.className =
    `history-item ${historyClass(
      multiplier,
    )}`;

  item.dataset.multiplier =
    String(multiplier);
  ...
  while (
    crashHistory.children.length > 15
  ) {
    crashHistory.lastElementChild
      ?.remove();
  }
}
...
function getBetPayload(slot) {
  ...
  if (
    ...
    if (
      ...
      showToast(
        "Auto cashout must be at least 1.01x.",
      );
...
function connectSocket() {
  ...
  socket.on(
    ...
    (payload) => {
      showToast(
        payload?.message ||
        "Round cancelled. Active bets were refunded.",
      );
