...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const API_URL = APP_CONFIG.api("/admin/withdraws");
  ...
  const refreshWithdraws = document.getElementById("refreshWithdraws");
  ...
  const pendingWithdrawCount = document.getElementById("pendingWithdrawCount");

  const approvedWithdrawCount = document.getElementById(
    "approvedWithdrawCount",
  );

  const rejectedWithdrawCount = document.getElementById(
    "rejectedWithdrawCount",
  );

  const paidToday = document.getElementById("paidToday");

  const pendingWithdrawMenuCount = document.getElementById(
  ...
  const approveAccount = document.getElementById("approveAccount");

  /* Reject fields */

  const rejectRequestId = document.getElementById("rejectRequestId");

  const rejectAmount = document.getElementById("rejectAmount");
  ...
  const detailLastFour = document.getElementById("detailLastFour");
  ...
  const detailPlayed = document.getElementById("detailPlayed");

  const detailTurnoverStatus = document.getElementById("detailTurnoverStatus");
  ...
  async function loadWithdrawals() {
    ...
    try {
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.message || "Withdraw requests load failed.");
      }
      ...
      showToast(error.message || "Could not load withdrawals.");
    } finally {
      hideLoader();
    }
  }
  ...
  function updateSummary() {
    ...
    const paidTodayAmount = approved
      .filter((item) => {
    ...
    if (pendingWithdrawCount) {
      pendingWithdrawCount.textContent = pending.length;
    }

    if (approvedWithdrawCount) {
      approvedWithdrawCount.textContent = approved.length;
    }

    if (rejectedWithdrawCount) {
      rejectedWithdrawCount.textContent = rejected.length;
    }

    if (paidToday) {
      paidToday.textContent = formatMoney(paidTodayAmount);
    }

    if (pendingWithdrawMenuCount) {
      pendingWithdrawMenuCount.textContent = pending.length;
    }
  }
  ...
  function getFilteredWithdrawals() {
    const search = String(withdrawSearch?.value || "")
      .toLowerCase()
      .trim();
    ...
    return withdrawals.filter((item) => {
      ...
      const searchMatched = !search || searchableText.includes(search);

      const statusMatched = status === "all" || item.status === status;

      const methodMatched = method === "all" || item.method === method;

      return searchMatched && statusMatched && methodMatched;
    });
  }
  ...
  function renderWithdrawals() {
    ...
    filtered.forEach((item) => {
      ...
      row.innerHTML = `
                <td class="user-cell">
                    <strong>
                        ${escapeHtml(item.fullName || item.username || "User")}
                    </strong>
                ...
                <td>
                    <span class="method-badge">
                        ${escapeHtml(String(item.method || "-").toUpperCase())}
                    </span>
                </td>
                ...
                <td class="account-cell">
                    ...
                    <small>
                        Last 4:
                        ${escapeHtml(item.lastFourDigits || "-")}
                    </small>
                </td>
  ...
  withdrawSearch?.addEventListener("input", renderWithdrawals);

  statusFilter?.addEventListener("change", renderWithdrawals);

  methodFilter?.addEventListener("change", renderWithdrawals);
  ...
  withdrawTableBody?.addEventListener("click", (event) => {
    ...
    const item = withdrawals.find((withdrawal) => Number(withdrawal.id) === id);

    if (!item) {
      showToast("Withdraw request not found.");
  ...
  function showDetails(item) {
    detailUser.textContent = item.fullName || item.username || "-";

    detailUserId.textContent = item.uid || "-";

    detailMethod.textContent = String(item.method || "-").toUpperCase();

    detailAccount.textContent = item.accountNumber || "-";

    detailLastFour.textContent = item.lastFourDigits || "-";

    detailRequestId.textContent = item.id;

    detailAmount.textContent = `৳${formatMoney(item.amount)}`;
    ...
    if (detailTurnoverStatus) {
      detailTurnoverStatus.textContent = "Server validated";
    }
  ...
  function openApprove(item) {
    approveRequestId.textContent = item.id;

    approveAmount.textContent = `৳${formatMoney(item.amount)}`;

    approveAccount.textContent = item.accountNumber;
    ...
    try {
      const response = await fetch(
        `${API_URL}/${selectedWithdrawal.id}/approve`,
        {
          ...
          body: JSON.stringify({
            adminNote: "JayaPay automatic payout approved by admin",
          }),
        },
      );
      ...
      showToast(error.message || "Could not approve withdrawal.");
    } finally {
      hideLoader();
  ...
  function openReject(item) {
    rejectRequestId.textContent = item.id;

    rejectAmount.textContent = `৳${formatMoney(item.amount)}`;
  ...
  confirmReject?.addEventListener("click", async () => {
    ...
    if (!reason) {
      showToast("Select a reject reason.");
    ...
    try {
      ...
      showToast(error.message || "Could not reject withdrawal.");
    } finally {
      hideLoader();
  ...
  refreshWithdraws?.addEventListener("click", async () => {
    await loadWithdrawals();

    showToast("Withdrawal requests refreshed.");
  });
  ...
  console.log("Admin Withdraw API Connected");
});
