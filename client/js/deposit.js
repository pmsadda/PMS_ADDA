document.addEventListener("DOMContentLoaded", () => {
  ...
  const senderNumberLabel =
  document.getElementById("senderNumberLabel");

const transactionIdLabel =
  document.getElementById("transactionIdLabel");

const binancePaymentSummary =
  document.getElementById("binancePaymentSummary");

const binanceRate =
  document.getElementById("binanceRate");

const binancePayableAmount =
  document.getElementById("binancePayableAmount");

const paymentQrWrapper =
  document.getElementById("paymentQrWrapper");

const paymentQrImage =
  document.getElementById("paymentQrImage");

const copyNumberText =
  document.getElementById("copyNumberText");
...
function formatAccountType(accountType) {
  const normalizedType =
    String(accountType || "personal").toLowerCase();
...
function updateBinancePayableAmount() {
  const paymentMethod =
    getActivePaymentMethod(selectedMethod);

  const isBinance =
    isBinanceMethod(paymentMethod?.method);
  ...
  const rate =
    Number(paymentMethod.bdtPerUsdt || 0);

  const amount =
    Number(amountInput?.value || 0);

  if (binanceRate) {
    binanceRate.textContent =
      rate > 0
        ? rate.toFixed(2)
        : "0.00";
  }
  ...
  if (binancePayableAmount) {
    binancePayableAmount.textContent =
...
function updatePaymentInputFields(paymentMethod) {
  ...
  if (senderNumberInput) {
    ...
    senderNumberInput.placeholder =
      isBinance
        ? "Enter your Binance Pay ID"
        : "01XXXXXXXXX";

    senderNumberInput.maxLength =
      isBinance ? 120 : 11;

    senderNumberInput.inputMode =
      isBinance ? "text" : "numeric";
  }

  if (transactionIdLabel) {
    transactionIdLabel.textContent =
      isBinance
  ...
  if (transactionIdInput) {
    ...
    transactionIdInput.placeholder =
      isBinance
...
function showPaymentUnavailable(message) {
  ...
  if (paymentMethodMessage) {
    ...
    paymentMethodMessage.classList.remove(
      "is-success",
    );

    paymentMethodMessage.classList.add(
      "is-error",
    );
  }

  if (paymentMethodTitle) {
    paymentMethodTitle.textContent =
      "Payment Method Unavailable";
  }

  if (merchantAccountType) {
    merchantAccountType.textContent =
      "Unavailable";
  }
  ...
  officialNumberCard?.classList.remove(
    "is-loading",
  );

  officialNumberCard?.classList.add(
    "is-unavailable",
  );
...
function selectPaymentMethod(method) {
  ...
  paymentCards.forEach((card) => {
    const isSelected =
      card.dataset.method === selectedMethod;
...
function renderPaymentMethods(paymentMethods) {
    ...
    .filter((paymentMethod) => {
      if (
        ![
          ...
          "rocket",
          "binance",
        ].includes(paymentMethod.method)
      ) {
        return false;
      }

      if (
        !Number.isInteger(paymentMethod.id) ||
        paymentMethod.id < 1
      ) {
        return false;
      }
  ...
  if (activePaymentMethods.length === 0) {
    showPaymentUnavailable(
      "Deposit payment method এখন পাওয়া যাচ্ছে না।",
    );
...
async function loadPaymentMethods() {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/deposits/payment-methods`,
      {
        headers: {
 ...
 copyNumberBtn?.addEventListener(
  ...
  async () => {
    ...
    try {
      ...
      showToast(
        "Receiving account copy করা যায়নি।",
      );
    }
  },
);
 ...
 submitBtn?.addEventListener(
  ...
  async () => {
    ...
    if (
      ...
      showToast(
        "একটি active payment method নির্বাচন করুন।",
      );
    ...
    if (
      ...
      showToast(
        "Deposit amount ৳100 থেকে ৳10,00,000-এর মধ্যে দিন।",
      );
    ...
    if (
      isBinance &&
      (
        senderNumber.length < 3 ||
        senderNumber.length > 120
      )
    ) {
      showToast(
        "সঠিক Sender Binance Pay ID দিন।",
      );
