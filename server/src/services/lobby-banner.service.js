...
const ALLOWED_STATUSES = new Set(Object.values(BANNER_STATUS));
...
function parseAdminId(value) {
  const adminId = Number.parseInt(value, 10);
...
function parseBannerId(value) {
  const bannerId = Number.parseInt(value, 10);

  if (!Number.isInteger(bannerId) || bannerId < 1 || bannerId > MAX_BANNERS) {
    throw createServiceError(
...
function normalizeStatus(value) {
  const status = String(value || BANNER_STATUS.DISABLED)
    .trim()
    .toLowerCase();
...
function normalizePopupMessage(value, showAsPopup) {
  const message = String(value || "")
    .replace(/\r\n/g, "\n")
    .trim();

  if (showAsPopup && !message) {
    throw createServiceError(
...
function normalizePopupButtonText(value) {
  const buttonText = String(value || "View Offer")
    .replace(/\s+/g, " ")
    .trim();
...
function detectBannerImageMimeType(buffer) {
...
function validateUploadedImage(file) {
   ...
   */
  const detectedMimeType = detectBannerImageMimeType(file.buffer);
  ...
  return {
    fileName: String(file.originalname || "lobby-banner").slice(0, 255),
...
function mapBannerRow(row) {
  ...
  return {
    ...
    title: row.title || "Lobby Banner",

    fileName: row.image_file_name || null,

    mimeType: row.image_mime_type || null,

    imageSize: row.image_size === null ? null : Number(row.image_size),
    ...
    popupButtonText: row.popup_button_text || "View Offer",
...
function findAvailableBannerId(rows) {
  const usedIds = new Set(rows.map((row) => Number(row.id)));

  for (let bannerId = 1; bannerId <= MAX_BANNERS; bannerId += 1) {
...
async function normalizeBannerOrder(connection) {
...
async function getBannerImage({ bannerId, includeDisabled = false } = {}) {
  const validBannerId = parseBannerId(bannerId);

  const conditions = ["id = ?", "image_data IS NOT NULL"];
...
async function createBanner({
  ...
  const validAdminId = parseAdminId(adminId);

  const validTitle = normalizeTitle(title);

  const validTargetUrl = normalizeTargetUrl(targetUrl);

  const validStatus = normalizeStatus(status);

  const validShowAsPopup = normalizeShowAsPopup(showAsPopup);

  const validPopupMessage = normalizePopupMessage(
    popupMessage,
    validShowAsPopup,
  );

  const validPopupButtonText = normalizePopupButtonText(popupButtonText);

  const uploadedImage = validateUploadedImage(imageFile);
  ...
  try {
    ...
    if (existingRows.length >= MAX_BANNERS) {
      throw createServiceError(
    ...
    const bannerId = findAvailableBannerId(existingRows);
    ...
    const displayOrder = existingRows.length + 1;
  ...
  `,
    ...
    return mapBannerRow(createdRows[0] || null);
  } catch (error) {
...
async function updateBanner({
  ...
  const uploadedImage = imageFile ? validateUploadedImage(imageFile) : null;
  ...
  try {
    ...
    const currentBanner = currentRows[0] || null;
    ...
    const hasBannerImage = Boolean(
      uploadedImage?.imageData || currentBanner.image_data,
    );

    if (validStatus === BANNER_STATUS.ACTIVE && !hasBannerImage) {
      throw createServiceError(
    ...
    return mapBannerRow(updatedRows[0] || null);
  } catch (error) {
    await connection.rollback();
...
async function deleteBanner({ bannerId, adminId }) {
  const validBannerId = parseBannerId(bannerId);
  ...
  try {
    ...
    await normalizeBannerOrder(connection);
